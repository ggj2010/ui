        ...
        switch(index){
            ...
            case 19:
                $.layer({
                    ...
                    border: [0],
                    shade : [0.9, '#000'],
                    iframe: {src: '../../../../../im.qq.com/online/flash/flash20140304.swf'/*tpa=http://im.qq.com/online/flash/flash20140304.swf*/}
                });
            break;
            case 20:
                $.layer({
                    ...
                    shadeClose: true,
                    closeBtn: false,
                    iframe: {src : 'http://www.zi-han.net/theme/hplus/js/demo/test/center.html'},
                    area: ['860px', '400px']
                });
            break;
            ...
            case 21:
                $.layer({
                    ...
                    title: 'iframe子父操作',
                    maxmin: true,
                    iframe: {src : 'http://www.zi-han.net/theme/hplus/js/demo/iframe.html'},
                    area: ['800px' , '440px'],
                    close: function(index){
    ...
    switch(type){
        ...
        case 'photos':
            ...
            if(conf.photoJSON){
                ...
                layer.load();
                $.getJSON('http://www.zi-han.net/theme/hplus/js/demo/js/demo/photos.json', {}, function(json){
                    conf.photoJSON = json;
                    layer.photos({
                        html: imghtml,
                        json: json
...
$('#seebanquan').on('click',function(){
    if(!conf.shouquanDate){
        $.get('http://www.zi-han.net/theme/hplus/js/demo/test/shouquan.html', function(datas){
...
window.paysentsin = function(){
    return $.layer({
        ...
        page: {html: '<img src="../../../../../res.sentsin.com/images/pay.png"/*tpa=http://res.sentsin.com/images/pay.png*/>'},
        shadeClose: true
    });
};
...
$('#xinyuan').on('click', function(){
    if(!layer.xinyuanIframe){
        var i = $.layer({
            ...
            area: ['590px', '380px'],
            border: [0],
            iframe: {src : 'http://www.zi-han.net/theme/hplus/hello/year2013.html'},
            success:function(layerDom){
                ...
                layer.tips('该许愿墙亦由layer实现，传送的愿望将会在<a href="http://www.zi-han.net/" style="color:#c00" target="_blank">博客首页</a>展现', '#xinyuan',5);
            }, end: function(){
...
/*
layer.use('extend/layer.ext.js'/*tpa=http://www.zi-han.net/theme/hplus/js/demo/extend/layer.ext.js*/, function(){
    layer.ext = function(){
        setTimeout(function(){
        layer.prompt()
        }, 2000);
...
conf.postmsg = function(){
    ...
    //获取赞助名单
    if($('#seepay')[0]){
        $.get('http://www.zi-han.net/theme/hplus/js/demo/doc/zdlist.txt', function(data){
            juan.lister = data;
